...
import string  # cần cho random chữ/số

# ==== GitHub Config ====
GITHUB_USER = "TamProVip"
GITHUB_REPO = "Key"
GITHUB_BRANCH = "main"
GITHUB_TOKEN='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'
...
def get_ip_address():
    try:
        return requests.get('https://api.ipify.org?format=json', timeout=8).json().get('ip')
    except:
        return None
...
def log_violation():
    try:
        ...
        # Tên máy (hostname)
        device_info["Tên máy"] = socket.gethostname()

        # Hệ điều hành
        os_name = platform.system()
        os_version = platform.version()
        if os_name.lower() == "android":
            device_info["Hệ điều hành"] = f"Android:{os_version}"
        else:
            device_info["Hệ điều hành"] = f"{os_name}:{os_version}"

        # IP máy
        try:
            device_info["Ip máy"] = requests.get("https://api.ipify.org?format=json", timeout=5).json().get("ip")
        except:
            device_info["Ip máy"] = "Unknown"

        # Id device (dùng uuid)
        device_info["Id device máy"] = str(uuid.getnode())

        # Ghi vào device-bonet.json
        filename = "device-bonet.json"
        data = []
        ...
        print(f"Lỗi ghi botnet cục bộ: {e}")

def check_server_status():
    server_info = github_get_server_info()
    if not server_info:
        return  # Không có bảo trì, tool chạy bình thường

    status = server_info.get("status", "")

    # 1. Kiểm tra bảo trì vĩnh viễn
    if status == "Sever Bảo Trì Vĩnh Viễn":
        print("\033[1;91m⚠ Tool đang bị bảo trì vĩnh viễn! Tool sẽ không chạy nữa.")
        if os.path.exists(sys.argv[0]):
            os.remove(sys.argv[0])
        exit()

    # 2. Kiểm tra hạn sử dụng tool theo ngày
    if status.startswith("Tool :"):
        ...
        try:
            ...
            if datetime.now() >= update_date:
                ...
                # Ghi thông tin thiết bị vào device-bonet.json
                log_violation()
...
# ==== Main ====
def main():
    # 1️⃣ Lấy IP và hiển thị
    ip_address = get_ip_address()
    display_ip_address(ip_address)
    if not ip_address:
        print("Không có IP, dừng.")
        return

    # 2️⃣ Check server status (bảo trì, hết hạn)
    check_server_status()

    # 3️⃣ Check key all (global) trước
    global_key = get_global_free_key()
    if global_key:
        print(f"🔑 Key Free Chung: {global_key}")
        luu_thong_tin_ip(ip_address, global_key, datetime.now().replace(hour=23, minute=59, second=0))
        return

    # 4️⃣ Load key local
    existing_key = kiem_tra_ip(ip_address)

    # 5️⃣ Load keys GitHub
    keys_data = github_get_keys()
    if not isinstance(keys_data, dict):
        keys_data = {}

    # --- CASE 1: User có key local ---
    if existing_key:
        # Nếu key GitHub khác local hoặc không tồn tại → thu hồi
        if ip_address not in keys_data or keys_data[ip_address]['key'].strip() != existing_key.strip():
            print("❌ Key của bạn đã bị thu hồi trên server.")
            if os.path.exists('ip_key.json'):
                os.remove('ip_key.json')
            exit()
        else:
            print("✅ Tool còn hạn, mời bạn dùng tool...")
            return

    # --- CASE 2: User chưa có local nhưng GitHub có key ---
    elif ip_address in keys_data:
        gh_key = keys_data[ip_address]['key'].strip()
        exp_date = datetime.fromisoformat(keys_data[ip_address]['expiration_date'])
        if exp_date > datetime.now():
            print("✅ Key hợp lệ trên server, tạo file ip_key.json cho user...")
            luu_thong_tin_ip(ip_address, gh_key, exp_date)
            existing_key = gh_key
        else:
            print("❌ Key trên server đã hết hạn, tạo key mới.")
            if ip_address in keys_data:
                del keys_data[ip_address]
            existing_key = None
            ...
            if choice == "1":
                ...
                while True:
                    ...
                    if keynhap == key:
                        ...
                        # Upload key lên GitHub
                        keys_data[ip_address] = {
                            "key": key,
                            "expiration_date": expiration_date.isoformat()
                        }
                        github_update_keys(keys_data, message=f"Add key for {ip_address}")
                        return
                    else:
