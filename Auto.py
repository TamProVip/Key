...
GITHUB_TOKEN='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'
...
def get_fixed_device_id():
    ...
    info = {'device_id': device_id, 'machine_name': machine_name, 'platform': machine_platform}
...
# ==== GitHub API ====
def github_get_keys():
    headers = {"Authorization": f"token {GITHUB_TOKEN}"}  # <-- dùng token trực tiếp
    ...
    if r.status_code == 200:
        ...
        print("Không lấy được keys.json, status code:", r.status_code)
        print(r.text)
        return {}
...
def github_update_keys(data, message="Update keys"):
    """
    Cập nhật keys.json lên GitHub. 
    Nếu file chưa tồn tại, sẽ tạo mới.
    """
    headers = {
    ...
    if r_get.status_code == 200:
        try:
            ...
            print("Lỗi khi lấy sha:", e)
    elif r_get.status_code == 404:
        print("File keys.json chưa tồn tại, sẽ tạo mới.")
    else:
        print(f"Không lấy được file keys.json, status code: {r_get.status_code}")
        print(r_get.text)
        return False
    ...
    if sha:
        payload["sha"] = sha  # Chỉ thêm sha nếu file đã tồn tại

    r_put = requests.put(GITHUB_API_URL, headers=headers, json=payload)

    # In thông tin debug
    #print("GitHub PUT status code:", r_put.status_code)
    try:
        pass
    except:
        pass
...
def get_shortened_link_phu(url):
    try:
        ...
        return {"status": "error", "message": "Không kết nối dịch vụ rút gọn URL."}
    except Exception as e:
...
def main():
    ...
    # 2️⃣ Kiểm tra key riêng cho device
    existing_key = None
    exp_device = None
    if device_id in keys_data:
        try:
            ...
            if exp_device > datetime.now():
                ...
                github_update_keys(keys_data, message=f"Revoke expired key for {device_id}")
        except Exception as e:
            print(f"DEBUG: Lỗi key device {device_id}:", e)
